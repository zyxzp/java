...
import com.example.springboot.converter.OrderMaster2OrderDTOConverter;
import com.example.springboot.dao.OrderDetailDao;
import com.example.springboot.dao.OrderMasterDao;
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDTO create(OrderDTO orderDTO) {
        ...
        //3.写订单库 orderMaster 和 orderDetail
    ...
    public OrderDTO finish(OrderDTO orderDTO) {
        ...
        OrderMaster orderMaster = new OrderMaster();
        BeanUtils.copyProperties(orderDTO, orderMaster);
        if (orderMasterDao.save(orderMaster) == null) {
            log.error(" [完结订单] 订单状态更新失败，orderId={},orderStatus={}", orderDTO.getOrderId(), orderDTO.getOrderStatus());
            throw new SellException(ResultEnum.ORDER_STATUS_ERROR);
        }
        return orderDTO;
    }
    ...
    public OrderDTO paid(OrderDTO orderDTO) {
        ...
        BeanUtils.copyProperties(orderDTO,orderMaster);
        if(orderMasterDao.save(orderMaster)==null){
            log.error(" [订单支付完成] 更新失败，orderId={},payStatus={}", orderDTO.getOrderId(), orderDTO.getPayStatus());
            throw new SellException(ResultEnum.ORDER_UPDATE_FAIL);
        }
        return orderDTO;
    }
